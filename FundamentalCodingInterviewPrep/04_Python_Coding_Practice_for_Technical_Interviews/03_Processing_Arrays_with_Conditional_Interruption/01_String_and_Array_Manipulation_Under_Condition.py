...
# Solving the problem String and Array Manipulation Under Condition.
...
def solution(inputString, numbers):
  ...
  while i < len(inputString) and i < len(numbers) and sum_so_far < 100:
    ...
    if current_char in vowels:
      next_char = 'a' if current_char == 'u' else vowels[vowels.find(current_char) + 1]
    else:
      next_char = 'b' if current_char == 'z' else consonants[consonants.find(current_char) + 1]
    
    result_string += next_char
    sum_so_far += numbers[i] * 3
    i += 1

  print(f"Modified string: {result_string}, remainder numbers {numbers[i:]}")
